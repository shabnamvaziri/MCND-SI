...
double* x1;
double* w;
int        S, Sperim, iter, K, N, A, * from_node_arc, * to_node_arc, * source, * destination, sglobal, max_iter, * b;
int* B;
double* r, * f, * p, * d, * c, M, obj, * vc, rho, *demand, * pPerim;
...
void main(int argc, char* argv[])
{
	...
	double cputime;
	int i, j, k, s, h, l, MaxNumInst, numSample, ss;
	start = clock();
	M = 0;
	if (argc == 1) {
	...
	for (numSample = 1; numSample < MaxNumInst; numSample++) {
		...
		fprintf(Out, "\n%s | %s ;\t", argv[1], instance);
		fprintf(Out, "%d\t%d\t%d\t%d\t%d\t|\t", N, A, K, S, Sperim);
		fclose(Out);
		double tempM = 0;
		...
		for (k = 0; k < K; k++) {
			for (ss = 0; ss < Sperim; ss++) {
				demand[k] += d[k * Sperim + ss] * pPerim[ss];
			}
		}
