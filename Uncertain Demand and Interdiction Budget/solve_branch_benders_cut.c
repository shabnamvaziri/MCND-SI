...
double solve_branch_benders_cut(void)
{
	...
	pos_y = create_double_vector((A + K) * K * Sperim);
...
printf("BBC start of decision variables\n");
	...
	ctype = create_char_vector((A + K) + ((A + K) * K * Sperim) + S);
	...
	for (h = 0; h < A + K; h++) {
		for (k = 0; k < K; k++) {
			for (ss = 0; ss < Sperim; ss++) {
				pos_y[(h * K + k) + (ss * (A + K) * K)] = index1 + num_w_var;
				obj[index1] = vc[h] * rho * pPerim[ss];
...
printf("BBC start of constraints\n");

	//Add constraint 1 // outward flow - inward fow = 0, d, -d
	numrows = K * N * Sperim;
	numnz = 2 * (A + K) * K * N * Sperim;
	...
	for (k = 0; k < K; k++) {
		for (l = 0; l < N; l++) {
			for (ss = 0; ss < Sperim; ss++) {
				...
				matbeg[index1++] = index;
				for (h = 0; h < A + K; h++) {
					matind[index] = pos_y[(h * K + k) + (ss * (A + K) * K)];
	...
	for (ii = 0; ii < iter + 1; ii++) {
		...
		for (s = 0; s < S; s++) {
			for (k = 0; k < K; k++) {
				for (l = 0; l < N; l++) {
					if (destination[k] == l) {
						temp1[s] -= demand[k] * pi[(l * K + k) + (s * N * K)][ii];
					}
					else if (source[k] == l) {
						temp1[s] += demand[k] * pi[(l * K + k) + (s * N * K)][ii];
					}
...
printf("BBC constraints are added");
...
printf("Here is solved 1");
			numcols = CPXgetnumcols(env, lp);
printf("Here is solved 2");
			status = CPXlpopt(env, lp);
printf("Here is solved 3");
			i = CPXgetstat(env, lp);
			oldOBJlp = OBJlp;
			...
			for (sglobal = 0; sglobal < S; sglobal++) {
				...
				if (flag[sglobal] == 1) {
					...
					for (k = 0; k < K; k++) {
						for (l = 0; l < N; l++) {
							if (destination[k] == l) {
								tempRelax -= demand[k] * piVio[(l * K + k)];
							}
							else if (source[k] == l) {
								tempRelax += demand[k] * piVio[(l * K + k)];
							}
			...
			for (h = 0; h < A + K; h++) {
				...
				for (k = 0; k < K; k++) {
					for (ss = 0; ss < Sperim; ss++) {
						UBlp += y[(h * K + k) + (ss * (A + K) * K)] * vc[h] * rho * pPerim[ss];
					}
				}
			}
			for (s = 0; s < S; s++) {
