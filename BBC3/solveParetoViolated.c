...
double solveParetoViolated(double *temp_x)
{
	...
	CPXsetdblparam(env4, CPX_PARAM_EPGAP, 0.000001); // e-optimal solution (%gap)
	...
	double sum_alfa = 0;
	for (h = 0; h < A + K; h++){
		alphaVio_pareto[h] = x[index];
		if (alphaVio_pareto[h] > 0){
			//printf("alfa[%d]= %lf\n", h, alpha_pareto[(h*S + sglobal) + (iter*(A + K)*S)]);
			sum_alfa += alphaVio_pareto[h];
		}
		index++;
	}
	for (l = 0; l < N; l++){
		for (k = 0; k < K; k++){
			piVio_pareto[(l*K + k)] = x[index];
